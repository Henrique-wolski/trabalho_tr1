...
while True:
    metodo_deteccao_correcao = "Bit_paridade" #input("Método de detecção/correção de erro a ser aplicado (Bit_paridade, Checksum, CRC, Hamming):")
...
while True:
    metodo_enquadramento = "Contagem de caracteres" #input("Método de enquadramento a ser aplicado (Contagem de caracteres, Inserção de bytes, Inserção de caracteres):")
...
while True:
    modulacao_digital_desejada = "NRZ-Polar"    #input("Modulação digital a ser aplicada (NRZ-Polar, Manchester ou Bipolar):")
...
for i in range(len(quadros_modulados)):
    ...
    while not(flag):
        ...
        termino = i == len(quadros_modulados)
        time.sleep(0.01)
