...
def espera_ack(socket, flag):
    ...
    while True:
        ...
        if len(ack) == 8:
            ...
            if ack == '0':
                for _ in range(8):
                    socket.send(b'1')    # Confirmação do recebimento do nack
                break
            else:
                if flag:
                    for _ in range(8):
                        socket.send(b'0')    # Sinalização para finalizar o processo
                    break
                else:
                    for _ in range(8):
                        socket.send(b'1')    # Confirmação do recebimento do ack
                    break
