...
def espera_ack(socket):
    global ack

    while True:

        sinal = socket.recv(1)  # Recebimento do ack
        ack += sinal
        if len(ack) == 8:
            if ack.count(0) >= 4:
                ack = '0'
...
for i in range(len(quadros_modulados)):
    ...
    thread = threading.Thread(target = espera_ack, args = (transmissor))
    thread.start()
    ...
    while not(flag):
        ...
        for amostra in quadros_modulados[i]:
            ...
            transmissor.sendall(amostra.encode("utf-8"))    # Envio da amostra do sinal
